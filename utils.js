function FormulaForCalculationDistFromPointToPoint(x, y, x1, y1) {
  const r = 6371 // радиус Земли в [км]

  // Перевод координат в радианы
  const latitude1 = y * Math.PI / 180
  const latitude2 = y1 * Math.PI / 180
  const longitude1 = x * Math.PI / 180
  const longitude2 = x1 * Math.PI / 180
...
function FormulaForCalculationDistFromPointToLine(x, y, x1, y1, x2, y2) {
  const A = x - x1;
  const B = y - y1;
  const C = x2 - x1;
  const D = y2 - y1;
...
const findDistanceFromCurrentPointToPoints = (point, currentPoint) => {
  const pointX = point.geometry.x
  const pointY = point.geometry.y
  const currentPointX = currentPoint.geometry.x
  const currentPointY = currentPoint.geometry.y
  const distance = FormulaForCalculationDistFromPointToPoint(pointX, pointY, currentPointX, currentPointY)
  return distance
}

const findDistanceFromCurrentPointToPolyline = (polyline, currentPoint) => {
  const polylinePaths = polyline.geometry.paths[0]
  const pointX = currentPoint.geometry.x
  const pointY = currentPoint.geometry.y
  const distances = []
  for (let i = 0; i < polylinePaths.length - 1; i++) {
    const pathStart = polylinePaths[i]
    const pathEnd = polylinePaths[i + 1]
    distances.push(FormulaForCalculationDistFromPointToLine(pointX, pointY, pathStart[0], pathStart[1], pathEnd[0], pathEnd[1]))
  }
...
const findDistanceFromCurrentPointToPolygon = (polygon, currentPoint) => {
